...
--
-- Name: changestatuspeserta(); Type: FUNCTION; Schema: public; Owner: mandan
--

CREATE FUNCTION changestatuspeserta() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
 update tbpeserta_ujian set status_ujian_peserta=4 where nobp=NEW.nobp;
...
ALTER FUNCTION public.changestatuspeserta() OWNER TO mandan;

--
-- Name: selesaiujian(); Type: FUNCTION; Schema: public; Owner: mandan
--

CREATE FUNCTION selesaiujian() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
 update tbpeserta_ujian set status_ujian_peserta=5 where nobp=NEW.nobp;
...
ALTER FUNCTION public.selesaiujian() OWNER TO mandan;
...
CREATE TABLE tbmahasiswa (
    ...
    nobp character varying(15) NOT NULL,
    status_mahasiswa smallint DEFAULT '1'::smallint NOT NULL
);
...
CREATE VIEW lap_hasil_ujian AS
 SELECT a.id_hasil,
    ...
    a.nilai,
    b.nm_mahasiswa
   FROM (tbhasil_ujian a
     JOIN tbmahasiswa b ON (((a.nobp)::text = (b.nobp)::text)));
...
CREATE TABLE tbdosen (
    ...
    status_dosen smallint DEFAULT '1'::smallint NOT NULL
);
...
CREATE TABLE tbmatkul (
    ...
    status_matkul smallint DEFAULT '1'::smallint NOT NULL
);
...
CREATE TABLE tbujian (
    ...
    status_ujian smallint DEFAULT '1'::smallint NOT NULL,
    id_jujian smallint NOT NULL,
    id_jsoal smallint NOT NULL,
    tahun_akademik character varying(6) NOT NULL,
    id_kelas smallint
);
...
CREATE VIEW lap_ujian AS
 SELECT a.id_ujian,
   ...
   FROM (((((tbujian a
     ...
     JOIN tbjenis_soal f ON ((a.id_jsoal = f.id_jsoal)))
     JOIN tbkelas g ON ((a.id_kelas = g.id_kelas)));
...
CREATE TABLE tbpeserta_ujian (
    ...
    nobp character varying(15) NOT NULL,
    id_ujian character varying(40) NOT NULL,
    status_peserta smallint DEFAULT '1'::smallint NOT NULL,
    status_ujian_peserta smallint DEFAULT '3'::smallint NOT NULL
);
...
CREATE VIEW lap_peserta_ujian AS
 SELECT a.id_peserta,
    ...
    b.status_mahasiswa,
    a.status_peserta,
    status_peserta.nm_status AS nm_status_peserta,
    a.status_ujian_peserta,
    status_upeserta.nm_status AS nm_status_upeserta,
    c.status_ujian,
    status_ujian.nm_status AS nm_status_ujian,
    c.status_dosen,
    c.status_matkul
   FROM (((((tbpeserta_ujian a
     JOIN tbmahasiswa b ON (((a.nobp)::text = (b.nobp)::text)))
     JOIN lap_ujian c ON (((a.id_ujian)::text = (c.id_ujian)::text)))
     JOIN tbstatus status_upeserta ON ((a.status_ujian_peserta = status_upeserta.status)))
     JOIN tbstatus status_peserta ON ((a.status_peserta = status_peserta.status)))
     JOIN tbstatus status_ujian ON ((c.status_ujian = status_ujian.status)));
...
CREATE TABLE tbuser (
    ...
    status_user smallint DEFAULT '1'::smallint NOT NULL
);
...
--
-- Name: tbmatkul_id_matkul_seq1; Type: SEQUENCE OWNED BY; Schema: public; Owner: mandan
--

ALTER SEQUENCE tbmatkul_id_matkul_seq1 OWNED BY tbmatkul.id_matkul;
...
--
-- Name: tbsoal_ujian_id_sujian_seq1; Type: SEQUENCE OWNED BY; Schema: public; Owner: mandan
--

ALTER SEQUENCE tbsoal_ujian_id_sujian_seq1 OWNED BY tbsoal_ujian.id_sujian;
...
--
-- Name: tbstatus_status_seq; Type: SEQUENCE OWNED BY; Schema: public; Owner: mandan
--

ALTER SEQUENCE tbstatus_status_seq OWNED BY tbstatus.status;
...
ALTER TABLE ONLY tbmatkul ALTER COLUMN id_matkul SET DEFAULT nextval('tbmatkul_id_matkul_seq1'::regclass);
...
ALTER TABLE ONLY tbsoal_ujian ALTER COLUMN id_sujian SET DEFAULT nextval('tbsoal_ujian_id_sujian_seq1'::regclass);


--
-- Name: status; Type: DEFAULT; Schema: public; Owner: mandan
--

ALTER TABLE ONLY tbstatus ALTER COLUMN status SET DEFAULT nextval('tbstatus_status_seq'::regclass);
...
SELECT pg_catalog.setval('tbdosen_id_seq', 1, false);
...
1	KKKI13111-1025069101-20151	14101152610565	100
7	KKKI13111-1025069101-20151	14101152610544	0
8	KKKI12106-1012019001-20141	13101152610342	7
\.
...
SELECT pg_catalog.setval('tbhasil_ujian_id_hasil_seq', 8, true);
...
1	KKKI12106-1012019001-20141	14101152610565	Javascript	5
4	KKKI12106-1012019001-20141	14101152610551	sdasx	5
5	KKKI12106-1012019001-20141	13101152610342	xzcxz	5
\.
...
SELECT pg_catalog.setval('tbjawaban_id_jawaban_seq', 5, true);
...
SELECT pg_catalog.setval('tbkuliah_id_kuliah_seq', 1, false);
...
SELECT pg_catalog.setval('tbmahasiswa_id_seq', 1, false);
...
SELECT pg_catalog.setval('tbmatkul_id_matkul_seq', 1, false);
...
COPY tbpeserta_ujian (id_peserta, nobp, id_ujian, status_peserta, status_ujian_peserta) FROM stdin;
14101152610565-KKKI12106-1012019001-20141	14101152610565	KKKI12106-1012019001-20141	1	3
14101152610545-KKKI12106-1012019001-20141	14101152610545	KKKI12106-1012019001-20141	1	3
14101152610565-KKKI13111-1025069101-20151	14101152610565	KKKI13111-1025069101-20151	1	3
14101152610550-KKKI13111-1025069101-20151	14101152610550	KKKI13111-1025069101-20151	1	3
14101152610551-KKKI12106-1012019001-20141	14101152610551	KKKI12106-1012019001-20141	1	4
14101152610544-KKKI13111-1025069101-20151	14101152610544	KKKI13111-1025069101-20151	1	5
13101152610342-KKKI12106-1012019001-20141	13101152610342	KKKI12106-1012019001-20141	1	5
\.
...
SELECT pg_catalog.setval('tbpeserta_ujian_id_peserta_seq', 1, false);
...
1	Javascript adalah?	\N	2	100	Bahasa Pemrograman
2	JS kependekan dari?	\N	2	100	Javascript
3	zssx	\N	2	100	szxz
4	mnlfdndsfk	\N	2	0	asx d skknas
5	JS kependekan dari?	\N	2	10	Javascript
6	2+3=	[{"huruf":"A","isi_pilihan":"3"},{"huruf":"B","isi_pilihan":"5"}]	1	1	B
\.
...
SELECT pg_catalog.setval('tbsoal_id_soal_seq', 6, true);
...
2	NaN	2
3	NaN	3
5	KKKI12106-1012019001-20141	5
6	KKKI13111-1025069101-20151	6
\.
...
SELECT pg_catalog.setval('tbsoal_ujian_id_sujian_seq', 1, false);
...
SELECT pg_catalog.setval('tbsoal_ujian_id_sujian_seq1', 6, true);
...
COPY tbujian (id_ujian, kd_matkul, nidn, hari, mulai, selesai, deskripsi, status_ujian, id_jujian, id_jsoal, tahun_akademik, id_kelas) FROM stdin;
KKKI12106-1012019001-20141	KKKI12106	1012019001	2017-11-24	10:00:00	11:30:00	Ujian	1	1	2	20141	1
KKKI12116-1020059001-20151	KKKI12116	1020059001	2017-11-01	08:00:00	10:00:00	Ujjian	1	1	1	20151	2
KKKI13111-1025069101-20151	KKKI13111	1025069101	2017-11-24	08:00:00	10:00:00	Ujian	1	1	1	20151	10
\.
...
SELECT pg_catalog.setval('tbujian_id_ujian_seq', 1, false);
...
--
-- Name: changestatuspeserta; Type: TRIGGER; Schema: public; Owner: mandan
--

CREATE TRIGGER changestatuspeserta BEFORE INSERT ON tbjawaban FOR EACH ROW EXECUTE PROCEDURE changestatuspeserta();


--
-- Name: selesaiUjian; Type: TRIGGER; Schema: public; Owner: mandan
--

CREATE TRIGGER "selesaiUjian" AFTER INSERT ON tbhasil_ujian FOR EACH ROW EXECUTE PROCEDURE selesaiujian();
