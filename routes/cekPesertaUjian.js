...
router.get('/:id_tsoal/ujian/:id_ujian/peserta/:nobp',(req, res, next)=>{
    ...
    var query
    if(id_tsoal == 2){
        query = db.raw("SELECT lap_peserta_ujian.id_mahasiswa, lap_peserta_ujian.nobp, lap_peserta_ujian.nm_mahasiswa, CASE WHEN (hasil_ujian.jawaban IS NOT NULL) THEN true ELSE false END AS status, hasil_ujian.id_tsoal FROM (lap_peserta_ujian LEFT JOIN ( SELECT distinct on (tbjawaban.id_peserta) tbjawaban.id_jawaban, tbjawaban.id_ujian, tbjawaban.id_peserta, tbjawaban.jawaban, tbujian.id_tsoal FROM (tbjawaban JOIN tbujian ON ((tbjawaban.id_ujian = tbujian.id_ujian))) WHERE (tbjawaban.id_ujian = ?)) hasil_ujian ON ((lap_peserta_ujian.id_mahasiswa = hasil_ujian.id_peserta))) where lap_peserta_ujian.id_ujian=? and lap_peserta_ujian.nobp=?",[id_ujian,id_ujian,nobp])
    }else{
